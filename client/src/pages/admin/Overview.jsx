...
export default function Overview() {
  const { data, loading, error } = useFetch('/admin/summary', []);
  ...
  return (
    <div>
      <h1 className="font-display text-3xl text-navy">Platform overview</h1>

      <div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        <StatTile label="Total revenue" value={formatINR(data?.totalRevenue)} accent />
        <StatTile label="Total orders" value={data?.orderCount ?? 0} />
        <StatTile label="Active artisans" value={data?.activeArtisans ?? 0} />
        <StatTile label="Pending artisans" value={data?.pendingArtisans ?? 0} />
        <StatTile label="Published products" value={data?.publishedProducts ?? 0} />
        <StatTile label="Registered buyers" value={data?.userCount ?? 0} />
      </div>
        ...
        <div className="mt-4">
          <ResponsiveContainer width="100%" height={300}>
            <BarChart data={chartData} margin={{ top: 8, right: 8, bottom: 0, left: -16 }}>
              ...
              />
              <Bar dataKey="orders" fill="#C9622B" radius={[4, 4, 0, 0]} maxBarSize={56} />
            </BarChart>
          </ResponsiveContainer>
        </div>
      </div>
